...
if __name__ == "__main__":
    LOGGER_FILE = "lgbBaseChecker_v5.log"
    MODEL_ID = "cleanedv5_iter1"
    NFOLDS=5
    ...
    IMAGE4_FEATS = ['image4_imt_top1_prob', 'image4_imt_top2_prob',
                    'image4_imt_top3_prob', 'image4_diff_top_1_label', 'image4_diff_top_2_label', 'image4_diff_top_3_label']
    ...
    features = BASE_FEATURES + CONT_COLS + PRICE_COMB_COLS + PRICE_MEAN_COLS + COUNT_COLS + IMAGE_FEATS + GIBA_FEATS + \
               IMAGE3_FEATS + IMAGE4_FEATS +TEXT_STATS + EXTRA_FEATS + EXTRA_FEATS2 + TEXT_PREDS + CAT_PREDS + RANK_PRED
    shuffle(features)
...
#    X_imt_bins = cross_val_predict(imt_deal_count, train, y=train["deal_probability"], cv=cvlist, method="transform")/int((1- 1/NFOLDS)* len(train))
 #   X_imt_bins_test = imt_deal_count.fit(train).transform(test) / len(train)
    ...
    logger.info(f'shape of X is {X.shape} and shape of y is {y.shape}')
