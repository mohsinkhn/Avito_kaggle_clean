...
def outoffold_crossvalidator_sparse(model, X, y, cvlist, callbacks=[], X_test=None, predict_test=False,
                             scorer=rmse, check_filename='tmp', multiinput=True, multioutput=False):
    if multioutput:
        y_preds = np.zeros(y[0].shape)
    else:
    ...
    for tr_index, val_index in cvlist:
        calls = copy.copy(callbacks)
        if multiinput:
            X_tr, X_val = [x[tr_index].tocsr() for x in X], [x[val_index, :].tocsr() for x in X]
        else:
            X_tr, X_val = X[tr_index].tocsr(), X[val_index].tocsr()
