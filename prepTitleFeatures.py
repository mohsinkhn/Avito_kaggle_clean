...
if __name__ == "__main__":
    ...
    COMBINATIONS = [
        (False, False, True, True, "l2", 1, True, True, False), #Best combination
        (True, True, True, True, "l1", 10, True, True, True),
    ...
    for i, comb in enumerate(COMBINATIONS):
        ...
        X = pipe.fit_transform(train["title"].astype(str))
        X_test = pipe.transform(test["title"].astype(str))
        logger.info("Shape of data {} and {}".format(X.shape, X_test.shape))

        logger.info("Saving features")
        save_npz("../utility/X_train_title_{}.npz".format(i), X)
        save_npz("../utility/X_test_title_{}.npz".format(i), X_test)
        logger.info("Done")
